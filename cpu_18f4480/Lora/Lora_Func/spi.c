...
void WriteSPI_ADDR_n( unsigned char addr,unsigned char *buffer, unsigned char size )
{
	unsigned char	i;
...
void ReadSPI_ADDR_n( unsigned char addr,unsigned char *buffer, unsigned char size )
{
	unsigned char	i;
...
void InitSPI(void)
{
	...
	TRISC5=0;   //sdo
	TRISA5=0;   //ss
	SSPSTAT = 0x40;         //SPI Bus mode 0,0
	SSPCON1 = 0x21;         //Enable SSP, Fosc/16

	CKE=0;  //0
	CKP=0;

	LATC4=1; // sdi pull high
	RC4=1;   // sdi pull high
}
