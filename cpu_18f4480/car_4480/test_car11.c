...
unsigned int  OtherKeyCheck(unsigned char pt)
{ 
	unsigned char Onkey[4];

	if(!Fire)	return(0);

    Onkey[0]=(CarKey[0] | YourKey[0]);
    Onkey[1]=(CarKey[1] | YourKey[1]);
    Onkey[2]=(CarKey[2] | YourKey[2]);
    Onkey[3]=(CarKey[3] | YourKey[3]);

	if(newscan[pt] == 0)	return(0);
	if((Onkey[0]==0) && (Onkey[1]==0) && (Onkey[2]==0) && (Onkey[3]==0))	return(0);
	if(newscan[pt] == Onkey[pt])	return(0);
