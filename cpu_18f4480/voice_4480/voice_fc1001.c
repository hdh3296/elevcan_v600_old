...
#define MENT_WATER FLOOR_B7+94	//102
...
#define MSY_WATER_SENSING 49
#define BEEP_DELAY_TIME 4000 // 단위 msec
volatile const unsigned char FloorChar[] = {
    "B5B4B3B2B10102030405060708091011121314151617181920212223242526272829303132"
};


unsigned char new_buf[64]; /*unlike the old_text this is not a CONST -> stored in data RAM */
far unsigned char * source_ptr_1 = (far unsigned char *)

                                   new_buf; /*pointers to data*/
far unsigned char * dest_ptr_1 = (far unsigned char *)
...
unsigned char CurCarKey[4] = {
    0, 0, 0, 0
};


// 현재 등록된 카키 콜 상태 저장
unsigned char BefCarKey[4] = {
    0, 0, 0, 0
};
...
unsigned char GetVoice_State(unsigned char, unsigned char);
...
void main(void) {
    ...
    while (1) {
        ...
        if (ELE_bIN_BUZ) {
            ...
            if (bDingdong == FALSE) {
                TmpCurVoice = GetVoice_OpenCloseUpDn(TmpCurVoice);
            }
            ...
            if (bSetSong) {
                TmpCurVoice = GetVoice_Song(TmpCurVoice);
            }
        }
        ...
        if (TmpCurVoice != NO_MENT) {
            ...
            if (bVoicePlaying) {
                ...
                if (((CurVoice >= START_FL) && (CurVoice <= END_FL)) || (CurVoice == FLOOR_P)) {
                    // 층 도착 !
...
void interrupt isr(void) {
    ...
    if (TMR0IF) {
        ...
        if (BeefDelayTimer < 0xffff) {
            BeefDelayTimer++;
        }

        if (RealayTestTimer < 0xffff) {
            RealayTestTimer++;
        }
...
unsigned char GetVoice_OpenCloseUpDn(unsigned char xTmpCurVoice) {
    ...
    if (ELE_bAUTO) {
        ...
        if ((ELE_bOUT_OP) && (xbOpened == FALSE) && !bVoicePlaying) {
            //open
            ...
            xbOpened = TRUE;
            UpDnVoiceTimer = 0;
        } else if (xbOpened && ELE_bOUT_CL) {
            // close : 문이 이미 열려있는 상태에서 close 출력이 나오면
...
bool isWarterState() {
	...
	return ELE_mSYSSTATUS == MSY_WATER_SENSING;
}

bool isWaterEmptyMent(UCHAR now_ment) {

	bool waterState = isWarterState();
	bool emptyMent = (now_ment != MENT_WATER);

    return waterState && emptyMent;
}


bool isWarterMentEnable() {
	bool autoMoving = (ELE_bCAR_MOVE && ELE_bAUTO);
	bool enable = FALSE;
...
unsigned char GetVoice_State(UCHAR returnMent, UCHAR nowMent) {

	static unsigned char EmergencyVoiceCnt = 0;

    if (returnMent != NO_MENT) {
        return returnMent;
    }

    // 파킹
    if (ELE_bPARKING_READY && (nowMent != PARKING_MENT)) {
        returnMent = PARKING_MENT;
    }
    ...
    if (ELE_bFIRE && (nowMent != HWAJAE_MENT) && bHajaeMentEn) {
        returnMent = HWAJAE_MENT;
    }
...
unsigned char GetVoice_CarCall(UCHAR xTmpCurVoice, UCHAR * xCurkey, UCHAR * xBefkey) {
    ...
    for (iFloor = 0; iFloor < 32; iFloor++) {
        if (xCurkey[j] != xBefkey[j]) {
            //8개의 카키 상태 비교
            // 한 개의 카키 상태 비교
            if ((xCurkey[j] &bitKey) && ! (xBefkey[j] &bitKey)) {
                // 카콜 등록이면? (참 && !거짓)
                if (iFloor != (ELE_nCURFLOOR - 1)) {
                    // 카콜 등록 층이 현재 층과 같지 않을 때는 출력을 내보낸다.
                    xBefkey[j] = (xBefkey[j] | bitKey);
                    xTmpCurVoice = GetCarCallMent(iFloor);
                }

                break;
            } else if (! (xCurkey[j] &bitKey) && (xBefkey[j] &bitKey)) {
                //카콜 취소이면? (!거짓 && 참)
                xBefkey[j] = (xBefkey[j] &~bitKey);
...
void SetCarKeyCancel(void) {
    ...
    if (bBefBtnClr == ON) {
        CurCarKey[0] = Elevator0.CarKey[0] = Elevator1.CarKey[0] = Elevator2.CarKey[0] = Elevator3.CarKey[0] =
                BefCarKey[0] = 0;
        CurCarKey[1] = Elevator0.CarKey[1] = Elevator1.CarKey[1] = Elevator2.CarKey[1] = Elevator3.CarKey[1] =
                BefCarKey[1] = 0;
        CurCarKey[2] = Elevator0.CarKey[2] = Elevator1.CarKey[2] = Elevator2.CarKey[2] = Elevator3.CarKey[2] =
                BefCarKey[2] = 0;
        CurCarKey[3] = Elevator0.CarKey[3] = Elevator1.CarKey[3] = Elevator2.CarKey[3] = Elevator3.CarKey[3] =
                BefCarKey[3] = 0;
    }
...
unsigned char VoiceBusy(void) {
    if (SPI_Play_Status_Chk()) {
        // 음성멘트 방송중이면 OUT1 LED에 불이 들어온다.
        return TRUE;
    } else {
        return FALSE;
    }
