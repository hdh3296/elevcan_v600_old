...
#define         UP_VOICE_X              	1
#define         DOWN_VOICE_X            	2
#define         OPEN_VOICE_X      	  	    3
#define         CLOSE_VOICE_X           	4
#define         OVERLOAD_VOICE_X        	5
#define         FLOOR_PARKING_X             6
#define         EMERGENCY_VOICE_X       	7
#define         HWAJAE_VOICE_X          	8
#define         POWER_DOWN_VOICE_X      	9
#define         DINGDONG_X              	10
#define         FLOOR_1_X                 11
#define         FLOOR_2_X                 12
#define         FLOOR_3_X                 13
#define         FLOOR_4_X                 14
...
#define         FLOOR_8_X                 18
#define         FLOOR_9_X                 19
#define         FLOOR_NONE_X              20
#define         FLOOR_xx_X                21
#define         FLOOR_xx10_X              22
#define         FLOOR_xx20_X              23
#define         FLOOR_xx30_X              24
#define         FLOOR_M_X                 25
#define         FLOOR_F_X                 26
#define         FLOOR_L_X                 27
...
unsigned    char     HwajaeVoiceCnt=0;
...
unsigned char	Serial_Status_Voice_Check(unsigned char id)
{
   ...
   if(!(RcvBuf[IdPt+1] & 0x20)){		//manual
      if(RcvBuf[IdPt+2] & 0x02){
         if(!updown_valid){
            if((UpDnVoice != UP_VOICE_X) && (RcvBuf[IdPt+1] & 0x80)){
               CurVoice=UpDnVoice=UP_VOICE_X;
               updown_valid=1;                                
            }
            if((UpDnVoice != DOWN_VOICE_X) && (RcvBuf[IdPt+2] & 0x01)){
               CurVoice=UpDnVoice=DOWN_VOICE_X;                                
               updown_valid=1;                                
            }
         }       
      }
...
/*
   if( (RcvBuf[IdPt+4] & S4_PARKING_READY)  && (SelVoice != FLOOR_PARKING_X)){
		CurVoice=FLOOR_PARKING_X;
   }
   if( (RcvBuf[IdPt+3] & S3_PARKING)  && (SelVoice == FLOOR_PARKING_X)){
   		if(VoiceRun != 0){
			Stop_Play();
			VoiceRun = 0;
		}
...
*/
   ...
   if(((RcvBuf[IdPt+1] & 0x01) || (RcvBuf[IdPt+3] & S3_OPEN_SUB)) && (RcvBuf[IdPt+1] & 0x04) && (SelVoice != OVERLOAD_VOICE_X) ){  
      CurVoice=OVERLOAD_VOICE_X;
   }
         
//   if((RcvBuf[IdPt+2] & 0x10) && (RcvBuf[IdPt+1] & 0x01)){
   if((RcvBuf[IdPt+2] & 0x10)){
		if(SelVoice != HWAJAE_VOICE_X){                
		  	if(HwajaeVoiceCnt < 2){
				CurVoice=HWAJAE_VOICE_X;
//				HwajaeVoiceCnt++;
			}
		}
		else	CurVoice=0xff;							
   }
   if( !(RcvBuf[IdPt+2] & 0x10))	HwajaeVoiceCnt=0;                



   if((RcvBuf[IdPt+1] & 0x08)  && (SelVoice != EMERGENCY_VOICE_X) ){
		if(EmergencyVoiceCnt < 10)	EmergencyVoiceCnt++;
	  	if(EmergencyVoiceCnt < 6)	CurVoice=EMERGENCY_VOICE_X;
   }
   if( !(RcvBuf[IdPt+1] & 0x08))	EmergencyVoiceCnt=0;
      
      
   if((RcvBuf[IdPt+1] & 0x10)  && (SelVoice != POWER_DOWN_VOICE_X) ){  
      CurVoice=POWER_DOWN_VOICE_X;
   }   
...
{      
   ...
   if( !(RcvBuf[IdPt+5] & 0x04)  && (SelVoice != POWER_DOWN_VOICE_X) && CarMove ){  
      CurVoice=POWER_DOWN_VOICE_X;
   }      
}
...
#else
//             if((CurVoice >= FLOOR_U5) && (CurVoice <= END_FL)){
              if((CurVoice >= FLOOR_1_X) && (CurVoice <= FLOOR_L_X)){					
#endif
                 //SelVoice=1;	//100909 - 남녀음악멘트 개발시 버그로 인해 잘못되었음을 알아서 막았습니다.
                 PlaySeq=10;
              }
...
//              if((CurVoice >= FLOOR_U5) && (CurVoice <= END_FL)){					
        ...
        if(PlaySeq==10){
           ...
           if(VoiceRun){
				...
				if(RcvBuf[IdPt+DSP1] == 'B'){
			    	PlaySeq=19;
				}
	 			else if(RcvBuf[IdPt+DSP1] == 'M'){
			    	PlaySeq=19;
				} 
				else	PlaySeq=20;
           }	
