...
unsigned    char  ParkingClearCount=0;
unsigned    char  FireClearCount=0;
...
unsigned int Earthquake(void)
{
    unsigned char earthquakeOn=0;
	...
	if(UpKeyBit){ 
   	...
   	if(CanTxAct){
		if(ReceiveAdr == LocalNumber){
   			if( StopBit && !earthquakeOn){
        		SelHostAdr=LocalNumber;                  
                CanCmd=CAN_EARTHQUAKE_COMMAND;
                CanKeyValue[1] = ((MyAddress-1) | CAR_READY);  
                CanKeyValue[2] = 0x0;  
				HostCallMe=0;              
                CanTx0();
			}
   			else if( !StopBit && earthquakeOn){
                CanCmd=CAN_EARTHQUAKE_COMMAND;
                CanKeyValue[1] = ((MyAddress-1) | CAR_READY);  
                CanKeyValue[2] = 0x2;  
				HostCallMe=0;              
...
unsigned int Parking_Fire(void)
{
    unsigned char parkingkey=0,fireman=0;
	...
	fireman=0;
   	...
   	if(CanTxAct){
		if(ReceiveAdr == LocalNumber){
   			if( Fire && !fireman && (FireClearCount < 100)){
				FireClearCount++;
        		SelHostAdr=LocalNumber;                  
                CanCmd=CAN_FIRE_COMMAND;
                CanKeyValue[1] = ((MyAddress-1) | CAR_READY);  
                CanKeyValue[2] = 0x0;  
				HostCallMe=0;              
                CanTx0();
			}
   			else if( !Fire && fireman){
                CanCmd=CAN_FIRE_COMMAND;
                CanKeyValue[1] = ((MyAddress-1) | CAR_READY);  
                CanKeyValue[2] = 0x2;  
				HostCallMe=0;              
   			...
   			else if( Parking && !parkingkey && (ParkingClearCount < 100)){
				ParkingClearCount++;
        		SelHostAdr=LocalNumber;                  
