...
#define     VIRTUAL_CALL_TIME    180  // 3 minute
...
unsigned    int   CallTimer[MAX_ELEV];
...
const   unsigned        char    WaitFlr[]={0x06,0x0c};


unsigned char  OtherElevator_Dest_1st_chk(unsigned char id,unsigned char flr)
{
	unsigned int   IdPt;
	unsigned char  ret;

	unsigned char  tmpdestflr,tmpdestflr_flr,tmpdestflr_Car;
	...
	tmpdestflr=RcvBuf[IdPt + DEST_FLR];
	tmpdestflr_flr=(tmpdestflr & ONLY_FLR) + 1;
	tmpdestflr_Car=(tmpdestflr & UPDN_CAR_READY);

	if(tmpdestflr_Car == CAR_READY ){
		if(!IsCarParking(IdPt) && IsCarAuto(IdPt) && IsCarMove(IdPt) && !IsCarFire(IdPt)){
			if(tmpdestflr_flr == flr){
				ret=1;
			}			
		}
	}
...
unsigned char  OtherElevator_1st_chk(unsigned char id,unsigned char flr)
{
	unsigned int   IdPt;
	unsigned char  ret;
	...
	if(IsCarFire(IdPt))			return(0);
	if(IsCarParking(IdPt)) 		return(0);
	if(IsCarMove(IdPt))			return(0);
	if( !IsCarAuto(IdPt))		return(0);
  	if( !IsCarNoneWard(IdPt))	return(0);

	
	if((RcvBuf[IdPt+S0_FLOOR] == flr)){
		ret=1;
	}
...
unsigned char  	EleAutoReadyTime(unsigned char id)
{
	unsigned int   IdPt;
	unsigned char  ret;
	...
	if(RcvBuf[IdPt+SL_mUnKnown06] != 65)	ret=1;	
	if(IsCarFire(IdPt))						ret=1;
	if(IsCarParking(IdPt)) 					ret=1;
	if(IsCarMove(IdPt))						ret=1;
	if( !IsCarAuto(IdPt))					ret=1;
  	if( !IsCarNoneWard(IdPt))				ret=1;
...
unsigned char  AutoReadyAndSetChk(void)
{
	unsigned char i,j;
	unsigned int  IdPt;
    ...
    for(i=0;i<MAX_ELEV;i++){
	    ...
	    if(RcvBuf[IdPt+SL_mUnKnown06] == 65){
		    if( IsCarClose(IdPt) && IsCarNoneWard(IdPt) && !IsCarParking(IdPt) && IsCarAuto(IdPt)  && !IsCarFire(IdPt)){
				if(CallTimer[i] > VIRTUAL_CALL_TIME){
					if( RcvBuf[IdPt+S0_FLOOR] == WaitFlr[0]){
		    			for(j=0;j<MAX_ELEV;j++){	
							if(j != i){
								if(OtherElevator_1st_chk(j,WaitFlr[0])){
									return(WaitFlr[1]);
								}	
								else if(OtherElevator_Dest_1st_chk(j,WaitFlr[0])){
									return(WaitFlr[1]);
								}	
							}
						}				
					}
					else if( RcvBuf[IdPt+S0_FLOOR] == WaitFlr[1]){
		    			for(j=0;j<MAX_ELEV;j++){	
							if(j != i){
								if(OtherElevator_1st_chk(j,WaitFlr[1])){
									return(WaitFlr[0]);
								}	
								else if(OtherElevator_Dest_1st_chk(j,WaitFlr[1])){
									return(WaitFlr[0]);
								}
							}
						}				
		    			...
		    			for(j=0;j<MAX_ELEV;j++){	
							if(j != i){
								...
								else if(OtherElevator_1st_chk(j,WaitFlr[1])){
									return(WaitFlr[0]);
								}	
								else if(OtherElevator_Dest_1st_chk(j,WaitFlr[1])){
									return(WaitFlr[0]);
...
//    for(;1;)    P0=0xf7;
    ...
    while(1){    
        ...
        if(CanTxAct){
			tmpkey=AutoReadyAndSetChk();
			if(tmpkey){
		 		CanCmd=CAN_KEY_SET;
		     	CanKeyValue[1] = ((tmpkey-1) | CAR_READY);
    ...
    if( TMR0IF )
    {
        ...
        if(msec100>100){
            ...
            if(sec1>10){
                ...
                for(i=0;i<MAX_ELEV;i++){          
                    MoveTime[i]++;
					if(CallTimer[i] < 1000)	CallTimer[i]++;
                }        
            }
        }
