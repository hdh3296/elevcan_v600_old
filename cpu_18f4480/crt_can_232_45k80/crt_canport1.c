...
#include "crt_canport1.h" 
#include "crt_comport1.h" 
//#include "com.h" 
//#include "crt_45k80.h" 
#include "MyUtil.h" 
...
unsigned int MasterCanLowDataLoad(unsigned char *src,unsigned char pt)
{
    uint8_t i;
	for( i = 0; i < 13; i++ )	Can1RxBuf[pt][i] = *(src +i);
	RxCanActive=1;
	return(0);
}

unsigned int SlaveCanLowDataLoad(unsigned char *src,unsigned char pt)
{
    uint8_t i;
	for( i = 0; i < 13; i++ )	Can1RxBuf[pt][i] = *(src +i);
	RxCanActive=1;
...
extern	uint16_t	CanLowDataLoad(uint8_t *buf);
...
unsigned int   Can1ReceiveData(void)
{
	...
	tmp[0]=(unsigned char)(Can1RxSid >> 8);
	tmp[1]=(unsigned char)(Can1RxSid);

	tmp[2]=(unsigned char)(Can1RxEid >> 16);
	tmp[3]=(unsigned char)(Can1RxEid >> 8);
	tmp[4]=(unsigned char)(Can1RxEid);

	xtmp=(unsigned char)(Can1RxDlc << 4);
	tmp[0]=(tmp[0] | xtmp); 


	for(i=0;i<8;i++)	tmp[5+i]=Can1ThisRxBuf[i];
...
/*
	if(Can1RxEid & 0x2000){
		...
		if(j>4)	j=4;	
		j=(j*13);
		for( i = 0; i < 13; i++ )	Can1RxBuf[j+i] = tmp[i];
		
		if(j>=52){
			Can1RxDataTotalCnt=65;
			Can1RxTimer=0;
		}
	}
	else{
		if(Can1RxDataTotalCnt>52)	Can1RxDataTotalCnt=0;

		if(Can1RxEid == 0x8b){
			for( i = 0; i < 13; i++ )	Can1RxBuf[i] = tmp[i];
			if(Can1RxDataTotalCnt==0)	Can1RxDataTotalCnt=13;		
		}
		else if(Can1RxEid < 0x80){
			for( i = 0; i < 13; i++ )	Can1RxBuf[Can1RxDataTotalCnt+i] = tmp[i];		
			Can1RxDataTotalCnt=(Can1RxDataTotalCnt+13);
		}
...
unsigned int   Can1ReceiveData(void)
{
	...
	if(Can1RxEid & 0x2000){				// Host data receive
		if(Can1RxEid & 0x08)	j=4;
		else					j=(unsigned char)(Can1RxEid & 0x07);
		j=(j * 8);
		...
		for(i=0;i<8;i++)	Can1RxBuf[j+i]=Can1ThisRxBuf[i];
		Can1RxDataRxPointer=(Can1RxDataRxPointer + 8);

		if(j==32){
      		for(j=0;j<Can1RxDataRxPointer;j++)	Com1TxBuffer[j+3]=Can1RxBuf[j];

			Com1TxBuffer[0]= 0x00;
			Com1TxBuffer[1]= 0xfe;
			Com1TxBuffer[2]= 0xa3;
			Com1TxBuffer[4]=Can1RxDataRxPointer;

 			Com1TxCnt=(Can1RxDataRxPointer+4);			
			Crc=0xffff;
			...
			OtherBufferLoad( &Com1TxBuffer,Com1TxCnt);
//			Com1TxStart();			
			Can1RxDataRxPointer=0;
		}
	}
...
unsigned int RfDataSetHeader(void)
{
	uint16_t	pt;	
	unsigned int tmpsid;
	unsigned int tmpsid;
	unsigned long tmpeid1,tmpeid2;


	if(Can1TxDataTotalCnt==0){
		Can1TxDataTxPointer=0;
		return(0);
	}
	
	if(Can1TxDataTxPointer>4){
		Can1TxDataTxPointer=0;
	...
	pt=(Can1TxDataTxPointer*13);

	Can1TxDlc=(Can1TxBuf[pt+0] >> 4);
	tmpsid=(Can1TxBuf[pt+0] & 0x0f);
	tmpsid=(tmpsid << 8);
	Can1TxSid=Can1TxBuf[pt+1];
	Can1TxSid=(Can1TxSid | tmpsid);

	tmpeid1=Can1TxBuf[pt+2];
	tmpeid1=(tmpeid1 << 16);
	tmpeid2=(Can1TxBuf[pt+3]);
	tmpeid2=(tmpeid2 << 8);
	Can1TxEid=Can1TxBuf[pt+4];

	Can1TxEid=(Can1TxEid | tmpeid1 | tmpeid2);

	if(!Can1TxData((pt+5))){
		if(Can1TxDataTotalCnt==((Can1TxDataTxPointer + 1) * 13)){
			Can1TxDataTotalCnt=0;
			Can1TxDataTxPointer=0;
		}	
		else	Can1TxDataTxPointer++;	
	}
	return(0);
}
