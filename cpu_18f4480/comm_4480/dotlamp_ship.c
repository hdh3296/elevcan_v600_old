...
//	if(RcvBuf[S2_STATE] & S2_LAMP_USER)			NoRunErr=1;					  // no service
//	else										NoRunErr=0;
#endif
...
//   	if((Rolling && (carbd==0)) || NoRunErr){
      ...
      for(i=0;i<=10;i++){
         if(j<k){ 
			if(Rolling && (carbd==0))			shiftDataBuf[i]=RollingP[j];
//			else if(NoRunErr)					shiftDataBuf[i]=NOSERVICE[j];
            j++;            
         }
         else{
			j=0;
...
else if((Fire && (carbd==0)) || (StopBit && (carbd==0)) ){
//      shift=(shift+1)%48;          
		k=40;  
      shift=(shift+1)%k;            
      j=shift;
